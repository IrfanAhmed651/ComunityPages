...
await mongoose.connect('mongodb://localhost:27017/communitypage', { useNewUrlParser: true, useUnifiedTopology: true, })
...
app.get('/comments', (req, res) => {
    ...
    const filters = search
        ? { body: { $regex: '.*' + search + '.*' } }
        : { rootId: null };
    Comment.find(filters).sort({ postedAt: -1 }).then(comment => {
        res.json(comment);
    });
});
